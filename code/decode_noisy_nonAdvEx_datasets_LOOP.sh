...
# Noise augmentation + decoding loop 
for ns in $noises; do
# "whitenoise" "pinknoise" 
    for snr in $snrs ; do
    #  "30"  "10" 
    echo -e " \n   Augmenting et_en_${data_basename} with $ns at $snr SNR"
    augm_data_dir=${orig_data_dir}/augmented_${ns}_${snr}SNR
    ...
    #recog_set=$advEx_wav_dir
        ...
        echo -e "\n       stage 2: extract new features from ${augm_data_dir} \n make_fbank_librosa.sh -> (new_augm_Data_)feats.scp"
    ...
    # fi
        ...
        echo -e "\n      stage 3: JSON file creation from the new feats.scp of ${augm_data_dir}"
    ...
    #fi
        ...
        decode_dir=decode_et_en_${data_basename}_augm_${ns}_${snr}SNR_$(basename ${decode_config%.*})
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            ...
            --backend ${backend} \
            --debugmode ${debugmode} \
            --recog-json ${augm_data_dir}/split${nj}utt/data.JOB.json \
            --result-label ${expdir}/${decode_dir}/data.JOB.json \
            --model ${expdir}/results/${recog_model}
