...
train_set=tr_${lang}_compr-128kbps_scaled0.7
train_dev=dt_${lang}_compr-128kbps_scaled0.7
recog_set="et_${lang}_compr-128kbps_scaled0.7" # " dt_${lang}_compr-128kbps_scaled0.7"
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    ########## IA: selection of wavs with different compression bitrates !!! (here 128 kbps)

    selected=${voxforge}/${lang}_compr_mp3_128kbps_scaled0.7/extracted


    # Initial normalization of the data
    local/voxforge_data_prep.sh ${selected} ${lang}_compr-128kbps_scaled0.7
    local/voxforge_format_data.sh ${lang}_compr-128kbps_scaled0.7
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 10 --write_utt2num_frames true \
        data/all_${lang}_compr-128kbps_scaled0.7 exp/make_fbank/train_${lang}_compr-128kbps_scaled0.7 ${fbankdir}
    utils/fix_data_dir.sh data/all_${lang}_compr-128kbps_scaled0.7
    ...
    remove_longshortdata.sh data/all_${lang}_compr-128kbps_scaled0.7 data/all_trim_${lang}_compr-128kbps_scaled0.7

    # following split consider prompt duplication (but does not consider speaker overlap instead)
    local/split_tr_dt_et.sh data/all_trim_${lang}_compr-128kbps_scaled0.7 data/tr_${lang}_compr-128kbps_scaled0.7 data/dt_${lang}_compr-128kbps_scaled0.7 data/et_${lang}_compr-128kbps_scaled0.7
    rm -r data/all_trim_${lang}_compr-128kbps_scaled0.7

    # compute global CMVN
    compute-cmvn-stats scp:data/tr_${lang}_compr-128kbps_scaled0.7/feats.scp data/tr_${lang}_compr-128kbps_scaled0.7/cmvn.ark
    ...
    dump.sh --cmd "$train_cmd" --nj 10 --do_delta ${do_delta} \
        data/tr_${lang}_compr-128kbps_scaled0.7/feats.scp data/tr_${lang}_compr-128kbps_scaled0.7/cmvn.ark exp/dump_feats/train ${feat_tr_dir}
    dump.sh --cmd "$train_cmd" --nj 4 --do_delta ${do_delta} \
        data/dt_${lang}_compr-128kbps_scaled0.7/feats.scp data/tr_${lang}_compr-128kbps_scaled0.7/cmvn.ark exp/dump_feats/dev ${feat_dt_dir}
...
dict=data/lang_1char_compr-128kbps_scaled0.7/tr_${lang}_units.txt
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    mkdir -p data/lang_1char_compr-128kbps_scaled0.7/
    echo "<unk> 1" > ${dict} # <unk> must be 1, 0 will be used for "blank" in CTC
    text2token.py -s 1 -n 1 data/tr_${lang}_compr-128kbps_scaled0.7/text | cut -f 2- -d" " | tr " " "\n" \
    | sort | uniq | grep -v -e '^\s*$' | awk '{print $0 " " NR+1}' >> ${dict}
    wc -l ${dict}

    # make json labels
    data2json.sh --lang ${lang} --feat ${feat_tr_dir}/feats.scp \
         data/tr_${lang}_compr-128kbps_scaled0.7 ${dict} > ${feat_tr_dir}/data.json
    data2json.sh --lang ${lang} --feat ${feat_dt_dir}/feats.scp \
         data/dt_${lang}_compr-128kbps_scaled0.7 ${dict} > ${feat_dt_dir}/data.json
    for rtask in ${recog_set}; do
...
expdir=exp/tr_en_compr-128kbps_scaled0.7_e4_12211_u320_proj320_d1_300_mtlalpha0.3_epo25_ss-0.5_19.01.2020
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    (
       ...
       #  splitjson.py --parts ${nj} ${feat_recog_dir}/data.json ##### IA. data.json is already corrupted with duplicates :(
        ...
        #for index in $(seq 9 16); do
            ${decode_cmd} JOB=9:16 ${expdir}/${decode_dir_advex}/log/decode.JOB.log \
                asr_recog_advex.py \
                --config ${decode_config_orig} \
                --ngpu ${ngpu} \
                --backend ${backend} \
                --debugmode ${debugmode} \
                --verbose ${verbose} \
